/*
File: edns.go
Version: 1.1.0 (Allocation Optimized)
Description: Handles all EDNS0 related logic.
             OPTIMIZED: 
             - addEDNS0Options now checks if changes are needed before allocating new slice.
             - Returns early for "preserve" modes if no changes are required.
*/
...
func addEDNS0Options(msg *dns.Msg, ip net.IP, mac net.HardwareAddr) {
	...
	// OPTIMIZATION: Check if we even need to process anything
	if ecsMode == "preserve" && macMode == "preserve" {
		return
	}
	...
	// Optimization: If modes are "add" and items exist, we can skip
	if ecsMode == "add" && hasECS && macMode == "add" && hasMAC {
		return
	}
	if ecsMode == "add" && hasECS && macMode == "preserve" {
		return
	}
	if ecsMode == "preserve" && macMode == "add" && hasMAC {
		return
	}
	...
	if ip != nil {
		...
		case "add":
			shouldAddECS = !hasECS
		case "replace":
			shouldAddECS = true // Will filter out existing later
		case "remove":
			shouldAddECS = false
		}
	}
	...
	case "replace":
		...
		macToAdd = determineMAC(mac, existingMAC, macSource)
	case "prefer-edns0":
		if !hasMAC && mac != nil && (macSource == "arp" || macSource == "both") {
			shouldAddMAC = true
			macToAdd = mac
		}
	case "prefer-arp":
		if mac != nil && (macSource == "arp" || macSource == "both") {
			...
			// If we prefer ARP but ARP is missing, and we have EDNS0, we might want to ensure we keep it
			// This logic depends on whether we filter or append. 
			// If "prefer-arp" implies "replace if ARP exists", then we act like replace.
			// If ARP is missing, we preserve.
		}
	}

	// If nothing to add and nothing to remove/replace, return
	if !shouldAddECS && !shouldAddMAC && ecsMode != "remove" && ecsMode != "replace" && macMode != "remove" && macMode != "replace" {
		return
	}

	// Rebuild Options Slice
	// Estimate capacity to avoid re-alloc
	newOpts := make([]dns.EDNS0, 0, len(o.Option)+2)

	// Filter existing
	for _, opt := range o.Option {
		switch v := opt.(type) {
		case *dns.EDNS0_SUBNET:
			if ecsMode == "replace" || ecsMode == "remove" {
				continue
			}
			newOpts = append(newOpts, opt)
		case *dns.EDNS0_LOCAL:
			if v.Code == EDNS0_OPTION_MAC {
				if macMode == "replace" || macMode == "remove" {
					continue
				}
				if macMode == "prefer-arp" && mac != nil {
					continue // Drop existing to replace with ARP
				}
				newOpts = append(newOpts, opt)
			} else {
				newOpts = append(newOpts, opt)
	...
	if shouldAddECS && ip != nil {
		...
		newOpts = append(newOpts, &dns.EDNS0_SUBNET{
	...
	if shouldAddMAC && macToAdd != nil {
		newOpts = append(newOpts, &dns.EDNS0_LOCAL{Code: EDNS0_OPTION_MAC, Data: macToAdd})
	}
