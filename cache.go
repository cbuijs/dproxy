...
func maintainDNSCache(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		case <-timer.C:
			prunedCount, prunedNX := pruneExpired()
			
			// Adaptive Timing Logic
			if prunedCount > 0 || prunedNX > 0 {
				// We found expired items, the cache is active/dirty.
				// Speed up maintenance to keep it clean.
				currentInterval = currentInterval / 2
				if currentInterval < minInterval {
					currentInterval = minInterval
				}
				if IsDebugEnabled() {
					LogDebug("[CACHE] Pruned %d items (%d NX). Accelerating cycle to %v", prunedCount, prunedNX, currentInterval)
				}
			} else {
				// Clean run. Relax the schedule.
				currentInterval = currentInterval * 2
				if currentInterval > maxInterval {
					currentInterval = maxInterval
				}
				if IsDebugEnabled() {
					LogDebug("[CACHE] Cache clean. Relaxing cycle to %v", currentInterval)
				}
			}
